...
def get_jenv():
    import frappe
    print("-----get_jenv-----no args")
    print("if not getattr(frappe.local, 'jenv', None):", getattr(frappe.local, "jenv", None))
    if not getattr(frappe.local, "jenv", None):
        ...
        print("UNSAFE_ATTRIBUTES", UNSAFE_ATTRIBUTES)
        class FrappeSandboxedEnvironment(SandboxedEnvironment):
            def is_safe_attribute(self, obj, attr, *args, **kwargs):
                print("check attr in UNSAFE_ATTRIBUTES", attr in UNSAFE_ATTRIBUTES)
                if attr in UNSAFE_ATTRIBUTES:
                    print("Output", False)
                    return False
                print("super().is_safe_attribute(obj, attr, *args, **kwargs)", super().is_safe_attribute(obj, attr, *args, **kwargs))
        ...
        print("---set the jenv in global", jenv)
...
def get_template(path):
    print("get_get_template", "path", path)
    print("output", get_jenv().get_template(path))
...
def render_template(template, context, is_path=None, safe_render=True):
    ...
    """
    print("utils>>jinja.py>>render_template()")
    print("=====args=====", template, context, is_path=None, safe_render=True)
    from jinja2 import TemplateError

    from frappe import _, get_traceback, throw

    if not template:
        print("=====template return empty=====")
        return ""

    if is_path or guess_is_path(template):
        print("=====is_path=====", is_path, "guess_is_path(template)", guess_is_path(template))
        print("output", get_jenv().get_template(template).render(context))
        return get_jenv().get_template(template).render(context)
    else:
        if safe_render and ".__" in template:
            print("if safe_render", safe_render,"and", ".__ in template", template)
            print("Illegal template")
            throw(_("Illegal template"))
        try:
            print("get_jenv().from_string(template).render(context)", get_jenv().from_string(template).render(context))
            return get_jenv().from_string(template).render(context)
        except TemplateError:
            print("Jinja Template Error")
            throw(
