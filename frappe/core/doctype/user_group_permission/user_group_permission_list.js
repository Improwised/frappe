frappe.listview_settings["User Group Permission"] = {
	onload: function (list_view) {
		var me = this;
		list_view.page.add_inner_button(__("Add / Update"), function () {
			let dialog = new frappe.ui.Dialog({
				title: __("Add User Group Permissions"),
				fields: [
					{
						fieldname: "user_group",
						label: __("For User Group"),
						fieldtype: "Link",
						options: "User Group",
						reqd: 1,
						onchange: function () {
				...
				primary_action: (data) => {
					...
					frappe.call({
						async: false,
						method: "frappe.core.doctype.user_group_permission.user_group_permission.add_user_group_permissions",
						args: {
							data: data,
						},
						callback: function (r) {
							if (r.message === 1) {
								frappe.show_alert({
									message: __("User Group Permissions created sucessfully"),
									indicator: "blue",
								});
							} else {
								frappe.show_alert({
		...
		list_view.page.add_inner_button(__("Bulk Delete"), function () {
			const dialog = new frappe.ui.Dialog({
				title: __("Clear User Group Permissions"),
				fields: [
					{
						fieldname: "user_group",
						label: __("For User Group"),
				...
				primary_action: (data) => {
					...
					frappe.confirm(__("Are you sure?"), () => {
						frappe
							.xcall(
								"frappe.core.doctype.user_group_permission.user_group_permission.clear_user_group_permissions",
								data
							)
							.then((data) => {
								dialog.hide();
	...
	get_applicable_doctype: function (dialog) {
		return new Promise((resolve) => {
			frappe
				.call({
					method: "frappe.core.doctype.user_group_permission.user_group_permission.check_applicable_doc_perm",
					async: false,
					args: {
		...
		return new Promise((resolve) => {
			frappe
				.call({
					...
					async: false,
					args: {
						user_group: dialog.fields_dict.user_group.value,
						doctype: dialog.fields_dict.doctype.value,
						docname: dialog.fields_dict.docname.value,
					},
				})
	...
	on_docname_change: function (dialog, options, applicable) {
		if (applicable.length != 0) {
			...
			dialog.set_title("Update User Group Permissions");
			...
			dialog.set_title("Add User Group Permissions");
			dialog.set_df_property("applicable_doctypes", "options", options);
			dialog.set_df_property("applicable_doctypes", "hidden", 1);
		}
		dialog.refresh();
